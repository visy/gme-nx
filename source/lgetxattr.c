...
#ifdef HAVE_GETXATTR
int lgetxattr(__const char *file_name, const char *name, 
	      void *value, size_t size)
{
  ...
  _zlibc_init();
  st=zlib_real_lgetxattr(file_name, name, value, size);
  ...
  errno = 0;  
  return zlib_real_lgetxattr(newname, name, value, size);
}
